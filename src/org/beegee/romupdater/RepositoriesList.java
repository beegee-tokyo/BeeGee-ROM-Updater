...
public class RepositoriesList extends ROMSuperActivity {
		...
		theList.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v, final int groupPosition, final int childPosition, long id) {
				AlertDialog.Builder alert = new AlertDialog.Builder(
						RepositoriesList.this);
				alert.setCancelable(false);

				// Model matches, it's a valid candidate
				if (rawList[groupPosition].getModel().equals(
						SharedData.LOCAL_MODEL)) {
					...
					alert.setPositiveButton(getString(R.string.yes),
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog,	int which) {
									...
									String url = rawList[groupPosition]
											.getRepositories()[childPosition]
											.getUrl();
									// trim the last character until it finishes
									// with "/" (HACK)
					...
					alert.setPositiveButton(getString(R.string.cancel),
							new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog,	int which) {
									dialog.dismiss();
	...
	void onJSONDataDownloaded(Boolean success) {
		...
		RepositoryExpandableListAdapter adapter = new RepositoryExpandableListAdapter(
				rawList);
